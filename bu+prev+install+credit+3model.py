...
# 根據經驗，LGB 與 CAT 表現較好，給 40% 權重；XGB 給 20%
blend_oof = (0.4 * lgb_oof) + (0.4 * cat_oof) + (0.2 * xgb_oof)
print(f"🎉 融合後的最終 OOF AUC: {roc_auc_score(y_train, blend_oof):.5f}")

# 5. 輸出最終 Submission 檔案
blend_preds = (0.4 * lgb_preds) + (0.4 * cat_preds) + (0.2 * xgb_preds)
sub = pd.DataFrame({'SK_ID_CURR': test_df['SK_ID_CURR'], 'TARGET': blend_preds})
sub.to_csv('ultimate_blend_submission.csv', index=False)