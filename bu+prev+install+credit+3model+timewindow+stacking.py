...
# 3. 初始化第二層模型 (Meta-Learner)
# 用邏輯斯迴歸（加上 L2 正規化防過擬合）
meta_model = LogisticRegression(C=1.0, solver='lbfgs', random_state=42)

# 4. 讓第二層模型去學習真正的 TARGET
meta_model.fit(meta_X_train, y_train)

# 5. 計算 Stacking 後的驗證集分數
stack_oof = meta_model.predict_proba(meta_X_train)[:, 1]
print(f"🔥 Stacking 終極 OOF AUC: {roc_auc_score(y_train, stack_oof):.5f}")

# 6. 輸出 Stacking 預測檔案
stack_preds = meta_model.predict_proba(meta_X_test)[:, 1]
sub_stack = pd.DataFrame({'SK_ID_CURR': test_df['SK_ID_CURR'], 'TARGET': stack_preds})
sub_stack.to_csv('ultimate_stacking_submission.csv', index=False)