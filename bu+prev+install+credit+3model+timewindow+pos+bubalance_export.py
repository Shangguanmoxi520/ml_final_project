...
# ========================================================
# 🚀 終極王牌：處理 bureau_balance.csv (信用局月度明細)
# ========================================================
print("正在處理 bureau_balance.csv...")
bb = pd.read_csv('home-credit-default-risk/bureau_balance.csv')

# 1. 對 STATUS 進行 One-Hot Encoding (展開為 STATUS_0, STATUS_1, STATUS_5 等)
bb = pd.get_dummies(bb, columns=['STATUS'], drop_first=False)

# 2. 第一層聚合：按「每筆貸款 (SK_ID_BUREAU)」計算月度摘要
bb_agg_rules = {
    'MONTHS_BALANCE': ['min', 'max', 'size'] # 貸款持續了幾個月
}
# 統計各個還款狀態出現的次數與比例
status_cols = [c for c in bb.columns if 'STATUS_' in c]
for col in status_cols:
    bb_agg_rules[col] = ['mean', 'sum'] # mean 代表該狀態出現的月份比例，sum 代表總月數
    

bb_agg = bb.groupby('SK_ID_BUREAU').agg(bb_agg_rules)
bb_agg.columns = pd.Index(['BB_' + e[0] + "_" + e[1].upper() for e in bb_agg.columns.tolist()])

# 3. 把第一層結果黏回 bureau 表
print("將 bureau_balance 合併至 bureau...")
...
# ========================================================
# 🚀 引入並處理最後一張子表 POS_CASH_balance.csv (含 6M 近期窗口)
# ========================================================
print("正在處理 POS_CASH_balance.csv...")
pos = pd.read_csv('home-credit-default-risk/POS_CASH_balance.csv')

# 1. 類別型欄位做 One-Hot Encoding (主要是合約狀態：Active, Completed, Signed 等)
pos_cat_cols = pos.select_dtypes(include=['object']).columns
pos = pd.get_dummies(pos, columns=pos_cat_cols, drop_first=True)

# 2. 定義數值型欄位的全局聚合規則
pos_num_aggregations = {
    'MONTHS_BALANCE': ['min', 'max', 'size'],  # 紀錄長度與合約月數
    'SK_DPD': ['max', 'mean'],                 # 當月逾期天數
    'SK_DPD_DEF': ['max', 'mean'],             # 經核可的逾期天數
    'CNT_INSTALMENT': ['max', 'mean'],         # 核准的分期期數規模
    'CNT_INSTALMENT_FUTURE': ['max', 'mean']   # 還剩下幾期沒繳
}

# 3. 對 One-Hot 展開後的類別欄位計算平均值
pos_cat_aggregations = {}
for col in pos.columns:
    if col not in pos_num_aggregations and col != 'SK_ID_CURR' and col != 'SK_ID_PREV':
        pos_cat_aggregations[col] = ['mean']

# 合併全局聚合規則
pos_agg_rules = {**pos_num_aggregations, **pos_cat_aggregations}

# 4. 執行全局 Groupby 聚合
pos_agg = pos.groupby('SK_ID_CURR').agg(pos_agg_rules)
pos_agg.columns = pd.Index(['POS_' + e[0] + "_" + e[1].upper() for e in pos_agg.columns.tolist()])

# --- 🚀 關鍵同步新增：近 6 個月的 POS 行為表現 ---
print("正在計算近 6 個月 POS 特徵...")
pos_6m = pos[pos['MONTHS_BALANCE'] >= -6]

if not pos_6m.empty:
    pos_6m_agg = pos_6m.groupby('SK_ID_CURR').agg({
        'SK_DPD': ['max', 'mean'],
        'CNT_INSTALMENT_FUTURE': ['mean']  # 最近半年平均還剩多少期，看近期負債壓力
    })
    pos_6m_agg.columns = pd.Index(['POS_6M_' + e[0] + "_" + e[1].upper() for e in pos_6m_agg.columns.tolist()])
    
    # 拼回全局聚合表
    pos_agg = pos_agg.join(pos_6m_agg, how='left')
...
print("POS_CASH_balance 處理並合併完成！")
...
# 創建一個字典，把訓練集、測試集、還有預測目標 TARGET 綁在一起
features_data = {
    'X_train_reduced': X_train_reduced,
    'X_test_reduced': X_test_reduced,
    'y_train': y_train  # 🚨 記得把你的 TARGET 標籤也存進去！
}

# 存成一個高壓縮的二進位檔案
joblib.dump(features_data, 'reduced_features_dataset.pkl', compress=3)
print("🎉 特徵導出成功！檔案名稱為: reduced_features_dataset.pkl")